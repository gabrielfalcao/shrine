...
for settings_key in dir(shrine_settings):
    ...
    if settings_key in ('DATABASES', 'TORNADO_CONFIG'):
        ...
        setattr(django_settings, settings_key,
                django_settings.PROJECT_PATH(settings_value))

    else:
        setattr(django_settings, settings_key, settings_value)

LOCAL_FILE = lambda *path: join(abspath(dirname(
    shrine_settings.__file__)), *path)
